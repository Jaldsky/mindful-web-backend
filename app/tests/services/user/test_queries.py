...
from app.services.user.queries import fetch_user_by_id
...
class TestUserQueries(TestCase):
    ...
    def test_fetch_user_by_id_returns_user(self):
        ...
        try:
            ...
            async def _test():
                ...
                async with manager.get_session() as session:
                    fetched = await fetch_user_by_id(session, user.id)
    ...
    def test_fetch_user_by_id_returns_none_for_missing(self):
        originals = self._patch_server_defaults_for_sqlite()
        try:
            ...
            async def _test():
                ...
                async with manager.get_session() as session:
                    fetched = await fetch_user_by_id(session, uuid4())
